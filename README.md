...
- 🐛 **MD5 动态签名破解** — 通过 JS 逆向分析，还原芒果 TV 搜索接口的 MD5 动态签名算法，成功突破数据采集限制，实现剧集数据的高效抓取与本地持久化，为应用提供稳定的数据来源支撑

- 🤖 **大模型流式对话** — 集成智谱 GLM-4.5 大语言模型，基于 SSE（Server-Sent Events）技术实现 AI 逐字流式输出，大幅提升对话交互体验，让用户感知更自然的实时响应

- 🎭 **双角色智能切换** — 支持「剧小迷」与「计算机专家」双角色自由切换，通过动态调整 System Prompt 实现不同风格的问答响应，满足娱乐推荐与技术咨询双场景需求

- 💾 **数据持久化存储** — 基于 MySQL 数据库存储用户对话记录、历史记录等核心数据，配合 JSON 字段灵活扩展存储结构，支持复杂数据模型，确保数据安全可靠

- 🔄 **触发器自动维护** — 通过 MySQL 触发器实现插入操作时自动记录当前时间戳，无需业务代码干预即可保证数据一致性与完整性

- 📱 **一套代码多端运行** — 基于 UniApp 跨端开发框架，一套 Vue 代码同时编译输出 H5 与微信小程序，大幅降低多端开发和维护成本

- 📊 **数据可视化分析** — 基于用户观看数据提取剧集标签，生成词云图展示热门标签分布，结合时序柱状图按时间维度分析观看趋势，为推荐优化提供数据支撑

- 🔒 **环境变量配置** — 数据库密码、API 密钥等敏感信息统一存放于 `.env` 文件，通过环境变量动态注入，从源头避免硬编码带来的配置泄露风险

- 📦 **前后端分离架构** — 前端采用 UniApp 负责界面交互与用户体，后端采用 Flask 提供功能 API 接口，职责边界清晰，便于团队协作与后期维护
